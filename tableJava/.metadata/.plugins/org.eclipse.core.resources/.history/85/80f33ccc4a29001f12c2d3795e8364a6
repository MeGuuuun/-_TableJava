...
<body>
    ...
    <c:choose>
    	<c:when test="${restaurant.r_status == '1' }">
    	...
    	<c:otherwise>
    		<button type="submit" name="book" disabled>예약 하기</button>
    	</c:otherwise>
    </c:choose>
    <c:choose>
    	<c:when test="${restaurant.w_status == '1' }">
    	...
    	<c:otherwise>
    		<button type="submit" name="wait" disabled>웨이팅 하기</button>
    	</c:otherwise>
    </c:choose>
</body>
</html>