...
<body>
	...
	<ul>
		<c:forEach items="${bookList}" var="book">
			<div>
				<li>예약한 식당 : ${book.restaurant_name } <br> 예약 날짜 :
					${book.date } <br> 예약 시간 : ${book.time } <br> 예약 인원 수 :
					${book.a_count + book.k_count } 명 <br> 유모차 사용 : ${book.s_count }
					개<br> 휠체어 사용 : ${book.w_count } 개<br> 요청 사항 :
					${book.requirement } <br>
				</li>
				<div>예약 상태 : ${book.status }</div>
				<div>
					...
					<c:if test="${book.status == '예약 확정' || book.status == '예약 취소' || book.status == '예약 거절' || book.status == '예약 종료' }">
