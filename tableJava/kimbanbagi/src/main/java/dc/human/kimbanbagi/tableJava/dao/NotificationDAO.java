...
public class NotificationDAO {
	...
	public int addWaitNotification(WaitDTO dto) {
		...
		// 웨이팅 차례가 되어 사용자를 호출하는 메소드
		// 사용자에게 웨이팅 호출 알림이 감
		public int confirmWait(String uId, String rName) {
			try {
				conn = DBConnectionManager.getConnection();
				...
				pstmt.setString(1, uId);
				pstmt.setString(2, rName);
				pstmt.setString(3, Comments.WAIT_C.getValue());
				pstmt.setDate(4, sqlDate);
				pstmt.setString(5, uId);
	...
	public int waitCancelFromUser(String uId, String rId, String rName) {
		...
		public int waitCancelFromOwner(String uId, String rId, String rName) {
			try {
				...
				if (row != 0) {
					...
					pstmt.setString(1, rId);
					pstmt.setString(2, rName);
					pstmt.setString(3, Comments.C_WAIT.getValue());
					pstmt.setDate(4, sqlDate);
					pstmt.setString(5, uId);
		...
		// 웨이팅 종료 메소드 (착석 완료)
		// 사용자와 사장님 모두에게 착석 완료 알림이 감
		public int endWait(String uId, String rId, String rName) {
			try {
				conn = DBConnectionManager.getConnection();
				...
				pstmt.setString(2, rId);
				pstmt.setString(3, rName);
				pstmt.setString(4, Comments.WAIT_E.getValue());
				pstmt.setDate(5, sqlDate);
				pstmt.setString(6, uId);
